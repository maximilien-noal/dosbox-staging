                      ...
                      const int port_choice)
{
	...
	LOG_MSG("INNOVATION: Running a SID %s at %0.3f MHz without filtering on port %xh",
	        model_choice.c_str(), chip_clock / 1000000.0,
	        base_port);
	else
	LOG_MSG("INNOVATION: Running a SID %s at %0.3f MHz with a %d%% filter on port %xh",
	        model_choice.c_str(), chip_clock / 1000000.0,
	        filter_strength,
	        base_port);
